...
class ControllerHelperBase:
    ...
    def convert_int_query_parameter(self, value: Union[str, None], param_name: str) -> Union[int, None]:
        """
        Converts request's query parameter to int.
        """
        if value:
            if value.isdigit():
                return int(value)
            else:
                raise RuntimeError(f'Parameter \'{param_name}\' has value \'{value}\', which cannot be converted to int')

        return None

    def convert_bool_query_parameter(self, value: Union[str, None], param_name: str) -> Union[bool, None]:
        """
        Converts request's query parameter to bool.
        """
        if value:
            if value.lower() == 'true':
                return True
                ...
                raise RuntimeError(f'Parameter \'{param_name}\' has value \'{value}\', which cannot be converted to bool')
